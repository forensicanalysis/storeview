<template>
  <div class="content">
    <svg
      ...
      :width="width"
      :height="height"
      :viewBox="-this.nodeRadius + ' ' + -this.nodeRadius + ' ' + (this.width + 2 * this.nodeRadius) + ' ' + (this.height + 2 * this.nodeRadius)"
      :id="id"
    />
...
<script>
  ...
  export default {
      ...
      graph: function () {
        ...
        for (let name in tasks) {
          if (tasks.hasOwnProperty(name)) {
            let task = tasks[name];
            if ('requires' in task) {
              task['requires'].forEach(function (requirement) {
    ...
    methods: {
      daggy: function (svgSelection, data, nodeRadius, height, width) {
        const defs = svgSelection.append('defs'); // For gradients

        let dag = d3.dagConnect()(data);
